...
auto main() -> int
{
    ...
    auto review = [&scheduler](const database::SpacedRepetitionData& srd, Rating rating) -> database::SpacedRepetitionData {
        auto srsAgain = scheduler.review(srd, Rating::fail);
        auto srsGood = scheduler.review(srd, Rating::pass);
        // auto srsEasy = scheduler.review(srd, Rating::familiar);
        spdlog::info("fail: {}", srsAgain.getDueInTimeLabel());
        spdlog::info("pass: {}", srsGood.getDueInTimeLabel());
