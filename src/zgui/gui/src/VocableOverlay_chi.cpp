...
VocableOverlay_chi::VocableOverlay_chi(std::shared_ptr<widget::Overlay> _overlay,
                                       std::shared_ptr<widget::TextToken> _token,
                                       std::shared_ptr<sr::DataBase> _database,
                                       Language language)
    : overlay{std::move(_overlay)}
    , word{std::dynamic_pointer_cast<database::Word_chi>(_token->getToken().getWord())}
...
auto VocableOverlay_chi::optionsFromWord(const database::Word_chi& word) -> std::vector<Option>
{
...
void VocableOverlay_chi::setupBox()
{
    overlay->start();
...
void VocableOverlay_chi::setupHeader(widget::Box& headerBox)
{
    using context::Image;
...
void VocableOverlay_chi::drawHeader(widget::Box& headerBox)
{
    headerBox.start();
...
void VocableOverlay_chi::setupDefinition(widget::Grid& definitionGrid)
{
...
void VocableOverlay_chi::drawDefinition(widget::Grid& definitionGrid)
{
...
void VocableOverlay_chi::setupOptions(widget::Box& optionBox)
{
    using context::Image;
    optionBox.clear();
...
void VocableOverlay_chi::drawOptions(widget::Box& optionBox)
{
    optionBox.start();
...
void VocableOverlay_chi::generateDefinitions()
{
    definitions.clear();
    for (const auto& option : options) {
        ...
        auto definition = database::Definition_chi();
...
auto VocableOverlay_chi::shouldClose() const -> bool
{
    return overlay->shouldClose();
}

auto VocableOverlay_chi::configured() const -> bool
{
