...
auto TextDraw::calculateDrift(Glib::RefPtr<Pango::Layout>& layout) const -> int
{
...
auto TextDraw::freshLayout() -> Glib::RefPtr<Pango::Layout>
{
...
auto TextDraw::sizeWithSpace(int width_or_height) const -> int
{
    return width_or_height + (spacing);
}

auto TextDraw::sizeNoSpace(int width_or_height) const -> int
{
    return width_or_height - (spacing);
}
...
auto TextDraw::mousePosToByteIndex(int x, int y) const -> int
{
    ...
    if (rect.get_x() > scaled_x || rect.get_x() + rect.get_width() < scaled_x || rect.get_y() > scaled_y || rect.get_y() + rect.get_height() < scaled_y)
        return -1;

    return byteIndex;
}

auto TextDraw::getWidth() const -> int
{
    return currentNaturalWidth;
}

auto TextDraw::getHeight() const -> int
{
