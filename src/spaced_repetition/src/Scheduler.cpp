                                   ...
                                   Rating rating) const -> SpacedRepetitionData
{
    ...
    case Rating::familiar:
        duration = nextIntervalDays(stability, ease);
                                    ...
                                    Rating rating) const -> SpacedRepetitionData
{
    ...
    case Rating::pass:
        ...
        if (lastDuration == learnSecondStepTime) {
            duration = nextIntervalDays(srd.stability, ease);
            studyState = StudyState::review;
        }
        break;
    default:
                                  ...
                                  Rating rating) const -> SpacedRepetitionData
{
    ...
    case Rating::fail:
        ...
        break;
    case Rating::pass:
        stability = nextStability(srd.stability, srd.ease, rating);
...
auto Scheduler::stabilityFromInterval(double interval, double ease) const -> double
{
    ...
    while (saveCount < 16 && std::abs(stability - oldStability) > 0.0001) {
        ...
        spdlog::info("i: {:.3F}, ni: {:.3F}, s: {}", interval, nextInterval(stability, ease), stability);
    }
...
auto Scheduler::nextInterval(double stability, double ease) const -> double
{
...
auto Scheduler::nextIntervalDays(double stability, double ease) const -> std::chrono::days
{
    double interval = nextInterval(stability, ease);
...
{
    constexpr double stabilizeAt = 4.;
                              ...
                              Rating rating) const -> int
{
    ...
    case Rating::fail: {
        ...
        double newInterval = nextInterval(failEase, failStability);
    ...
    case Rating::pass: {
        ...
        double intervalHard = nextInterval(stabilityHard, easeHard);
        double interval = nextInterval(stability, ease);
                             ...
                             Rating rating) const -> int
{
    ...
    case Rating::pass: {
        ...
        double intervalEasy = nextInterval(stabilityEasy, easeEasy);
        double interval = nextInterval(stability, ease);
        spdlog::info("ih{:.2F}:ig{:.2F}, dh{:.2F}, sh{:.2F}", intervalEasy, interval, easeEasy, stabilityEasy);
        return static_cast<int>(std::max((intervalEasy - interval), 0.));
        // double easeEasy = increaseEase(srd.ease, speedChange);
        //  easeEasy = increaseEase(easeEasy, speedChange);
